...
| D29 | A full match on a built name (or its plural or alias) still opens the page as you type; this is exact-match navigation, not typeahead, and it survives the D27 revision. Everything else waits for Enter | Decided |
| D50 | Without a dropdown there is nothing to pick from, so resolution is: exact name or alias, else a single unambiguous prefix match ("merk" resolves to merkle tree), else not supported. An ambiguous prefix such as "hash" is treated as not supported | Default (OQ-20) |
...
| R15 | Landing per D27: nothing appears while typing; Enter on an allowlisted name with no page gives the "real concept, not written and checked yet" message; Enter on anything else gives the "not a tech concept we can explain yet" message; both offer the built explainer; a full match on a built name opens it as you type. Works with network off | Type "merk" and watch: no message until Enter. Enter on "merkle tree", "hash map" (alias), "arrays" (plural), "merk" (unique prefix): real-concept message plus the Bloom button. Enter on gibberish, an emoji, 200 chars: not-supported message plus the same button. Empty Enter: nothing. Type "bloom filter" and "bloom filters": page opens. Repeat everything with network off |
...
| OQ-20 | Ambiguous prefixes. "hash" prefix-matches hash table, hash function and hashing, so D50 calls it not supported, which is a small lie: it is a real concept family. The alternative is a third message listing the candidates, which is a small dropdown by another name | Treat as not supported. Say if you want the third message |
